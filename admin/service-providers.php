...
if (isset($_GET['provider_id'])) {
    ...
    $chats = mysqli_query($conn, "SELECT * FROM `chats`
    ...
    if (isset($_POST['submit'])) {
        ...
        if (isset($massege) && $massege > 0) {
            $insert = mysqli_query($conn, "INSERT INTO `chats`(message, provider_id, admin_id, sender) VALUES('$massege', '$provider_id', '$admin_id', 'admin')") or die('query failed');
            header('location: service-providers.php?provider_id=' . $provider_id . '');
        } else {
            header('location: service-providers.php?provider_id=' . $user_id . '');
        }
    }
} else {
...
<head>
    ...
    <style>
        ...
        .chat-container {
            ...
            height: 100%; /* Full height to fill the main container */
        }
        .messages {
            flex-grow: 1; /* Allow messages area to grow */
            overflow-y: auto; /* Scroll for messages */
            max-height: 60vh; /* Maximum height for messages */
        ...
        <div class="users-list">
            ...
            if (isset($service_providers) && mysqli_num_rows($service_providers) > 0) {
                while ($service_provider = mysqli_fetch_assoc($service_providers)) {
                    ...
                    <a href="service-providers.php?provider_id=' . $id . '" style="color: black;text-decoration: none">
                        <div><img src="' . $img . '" alt="User 2">' . $service_provider['store_name'] . '</div>
                    </a>
                </div>';
                }
            } else {
                echo '<div class="user-item">
                <a href="service-providers.php" style="color: black;text-decoration: none">
