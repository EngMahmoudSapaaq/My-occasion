...
if (isset($_GET['provider_id'])) {
    ...
    $service_providers = mysqli_query($conn, "SELECT * FROM `service_providers` WHERE provider_id = '$provider_id'") or die('Query failed');
...
ORDER BY provider_id DESC;") or die('Query failed: ' . mysqli_error($conn));
    ...
    if (isset($_POST['submit'])) {
        ...
        if (isset($massege) && $massege > 0) {
            $insert = mysqli_query($conn, "INSERT INTO `chats`(message, provider_id, user_id ,sender) VALUES('$massege', '$provider_id', '$user_id', 'user')") or die('query failed');
...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
        ...
        <div class="container chat-container" style="border: 5px solid #DCD4B6;padding: 5px;border-radius: 8px;margin-top: 50px;margin-bottom: 50px">
            ...
            <div class="user-list">
                <div class="user-item">
                    <img src="<?php echo $img; ?>" alt="User 1">
                        <div>
                            <strong><?php echo $service_provider['store_name']; ?></strong>
                        </div>
